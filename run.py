...
from experiments.exp import Exp_mydata
import time

parser = argparse.ArgumentParser(description='SCINet on ETT dataset')
parser.add_argument('--model', type=str, required=False, default='mymodel', help='model of the experiment')
parser.add_argument('--data', type=str, required=False, default='mydata', choices=['ETTh1', 'ETTh2', 'ETTm1'], help='name of dataset')
parser.add_argument('--root_path', type=str, default='city-data/', help='root path of the data file')
parser.add_argument('--data_path', type=str, default='mydata.csv', help='location of the data file')
parser.add_argument('--features', type=str, default='M', choices=['S', 'M'], help='features S is univariate, M is multivariate')
parser.add_argument('--target', type=str, default='扬州_PM2.5', help='target feature')
parser.add_argument('--freq', type=str, default='h', help='freq for time features encoding, options:[s:secondly, t:minutely, h:hourly, d:daily, b:business days, w:weekly, m:monthly], you can also use more detailed freq like 15min or 3h')
parser.add_argument('--checkpoints', type=str, default='exp/mydata_checkpoints/', help='location of model checkpoints')
...
parser.add_argument('--seq_len', type=int, default=24, help='input sequence length of SCINet encoder, look back window')
parser.add_argument('--label_len', type=int, default=24, help='start token length of Informer decoder')
parser.add_argument('--pred_len', type=int, default=3, help='prediction sequence length, horizon')
...
data_parser = {
    'mydata':{'data': 'mydata.csv', 'T': '扬州_PM2.5', 'M': [15*18, 15*18, 15*18], 'S': [1, 1, 1], 'MS': [15*18, 15*18, 1]},
}
...
torch.manual_seed(753)  # reproducibleq
torch.cuda.manual_seed_all(753)
...
if args.evaluate:
    ...
    setting = '{}_k{}_w{}_{}_ft{}_sl{}_ll{}_pl{}_lr{}_bs{}_hid{}_l{}_dp{}_inv{}_itr0'.format(args.model,args.topk,args.weight,args.data, args.features, args.seq_len, args.label_len, args.pred_len,args.lr,args.batch_size,args.hidden_size, args.levels,args.dropout,args.inverse)
    ...
    if args.itr:
        ...
        for ii in range(args.itr):
            ...
            setting = '{}_k{}_w{}_{}_ft{}_sl{}_ll{}_pl{}_lr{}_bs{}_hid{}_l{}_dp{}_inv{}_itr{}'.format(args.model,args.topk,args.weight,args.data, args.features, args.seq_len, args.label_len, args.pred_len,args.lr,args.batch_size,args.hidden_size, args.levels,args.dropout,args.inverse,ii)
        ...
        print('Final mean normed mse:{:.4f},mae:{:.4f},denormed mse:{:.4f},mae:{:.4f}'.format(mse, mae, mses, maes))
        print('cost time:{:.2f}'.format(cost_time))
