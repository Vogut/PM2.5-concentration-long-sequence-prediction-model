...
class StandardScaler():
    ...
    def inverse_transform(self, data, target = 9*15):
        mean = torch.from_numpy(self.mean).type_as(data).to(data.device) if torch.is_tensor(data) else self.mean
        #mean = mean[0::15]
        mean = mean[target] 
        std = torch.from_numpy(self.std).type_as(data).to(data.device) if torch.is_tensor(data) else self.std
...
def load_cities(path="city-data", dataset="distances.xlsx"):

    '''Initialization edgs of adj, use the the Euclid distance between cities, and set threhold as '''
    # # ------------------------------------------------
    adj_path = f'{path}/{dataset}'
    adj_raw = pd.read_excel(adj_path)
    adj_raw = np.array(adj_raw)
    adj_init = []
    threshold = 200
    for i in range(len(adj_raw)):
        adj_init.append(np.array(adj_raw[i][1:], dtype=np.float64))
    adj = np.array(adj_init)
    adj = np.where(adj <= threshold, 1.0, 0.0)
    # # ------------------------------------------------

    '''Some preparation for data. '''
    # # ------------------------------------------------
